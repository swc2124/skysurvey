...
def table_merge():
    import astropy.table as astotable
    import ebf
    import sys
    xbox_min_value = 10.0
    ...
    satprop = ebf.read(os.path.join(
        Config.get('PATH', 'data_dir'), 'satprop.ebf'))
    for halo in Config.get('Default_halos', 'halos').split(','):
        print(halo)
        stellar_table_fh = os.path.join(
            table_dir, 'spinbin_output', halo + '_4.0Mpc_h158_table.hdf5')
        bin_table_fh = os.path.join(
            table_dir, 'table_bin_output', '4.0Mpc_h158_' + halo + '_table_bin_table.hdf5')

        bin_table = Table.read(bin_table_fh, format='hdf5', path='data')
        ...
        stellar_table = Table.read(
            stellar_table_fh, format='hdf5', path='data')
        print(stellar_table.info)
        stellar_table.keep_columns(['x_int', 'y_int', 'satids'])

        print('removing bound sats')
        for satid in stellar_table.meta['satids']:
            if satprop['bsat'][satid]:
                stellar_table.remove_rows(np.nonzero(stellar_table['satids'] == satid))
                print(' -> removing satid:', satid)

        print(stellar_table.info(['attributes', 'stats']))
        stellar_table.pprint(max_lines=25, max_width=window_size()[
                             0], show_name=True, show_unit=True, show_dtype=True, align=None)
        print('joining tables for', halo, '...')
        new_table = astotable.join(
            stellar_table, bin_table, join_type='right', keys=['x_int', 'y_int'])
        print('done')
        print(new_table.info(['attributes', 'stats']))
        new_table.pprint(max_lines=25, max_width=window_size()[
                         0], show_name=True, show_unit=True, show_dtype=True, align=None)
        new_table_fh = os.path.join(table_dir, 'merged_tables', halo + '.hdf5')
        print('writing new table to: ', new_table_fh)
        new_table.write(new_table_fh, format='hdf5', path='data', compression=True,
    ...
    for grid_fh in grid_files:
        ...
        for radius_Kpc in range(radius_start, radius_end, step):
            ...
            columns = [Column(data=BOXES.astype(np.float32), name='Boxes', description='Number of stars per grid box'),
                       ...
                       Column(data=XBOX_sqrt.astype(np.float16), name='Xbox_sqrt',
                              description='(Boxes - mu) / sigma where sigma=sqrt(Boxes.mean())', unit='float16'),
                       Column(data=XBOX_std.astype(np.float16), name='Xbox_std',
                              description='(Boxes - mu) / sigma where sigma=Boxes.std()', unit='float16'),
                       ...
                       Column(data=(x - center).astype(np.int16), name='X',
                              description='x indices on the grid - center', unit='int16'),
                       Column(data=(y - center).astype(np.int16), name='Y',
                              description='y indices on the grid - center', unit='int16'),
                       ...
                       Column(data=np.ones(n_boxes).astype(np.float32) * mu, name='Mu',
                              description='mu = Boxes.mean()', unit='float32'),
                       Column(data=np.ones(n_boxes).astype(np.uint32) * BOXES.sum(), name='Nstars_r',
                              description='Total strs in radial selection', unit='uint32'),
                       Column(data=np.ones(n_boxes).astype(np.float16) * np.float16(np.log10(BOXES.sum())), name='Log10(Nstars)_r',
                              description='Total strs in radial selection', unit='float16'),
                       Column(data=np.ones(n_boxes).astype(np.uint32) * n_boxes, name='N_boxes',
                              description='Total number of grid boxes at this R', unit='uint32'),
                       Column(data=np.ones(n_boxes).astype(np.float16) * np.float16(np.log10(n_boxes)), name='Log10(N_boxes_tot)',
                              description='Total number of grid boxes at this R', unit='float16'),
                       Column(data=np.ones(n_boxes).astype(
                           np.uint32) * len(np.nonzero(BOXES == 0.0)[0]), name='N_boxes_empty', unit='uint32'),
                       Column(data=np.ones(n_boxes).astype(
                           np.float16) * np.float16(np.log10(len(np.nonzero(BOXES == 0.0)[0]))), name='Log10(N_boxes)_empty', unit='float16'),
                       Column(data=np.ones(n_boxes).astype(
